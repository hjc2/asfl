...
class FedAdaptive(FedCustom):
    def __init__(
        ...
        *args,
        ...
        mu: float = 0.0,  # Proximal term parameter
        **kwargs,
    ):
        ...
        self.client_history = {}
        self.round_metrics = {}
        self.mu = mu  # Save the proximal term parameter
    ...
    def aggregate_fit(
        ...
        """Aggregate model weights using adaptive weighting strategy with proximal term."""
        if not results:
            return None, {}
        ...
        global_parameters = parameters_to_ndarrays(self.global_model.parameters)

        if self.mu > 0.0:
            # Proximal term: adjust the aggregated weights
            for i in range(len(aggregated_weights)):
                aggregated_weights[i] += self.mu * (global_parameters[i] - aggregated_weights[i])
